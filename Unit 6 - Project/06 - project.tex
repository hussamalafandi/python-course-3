...
\textbf{Bonus:} Allow adding multiple transactions in one function call (e.g., for batch import).

\subsection{2) List transactions}
The user can list all transactions. Minimum output: one line per transaction.

\textbf{Filtering (required):} Implement a flexible filter function that can accept \textbf{any combination} of criteria (e.g., filter by type only, by category only, by both, or by date range). The same function should handle all cases.

\textit{Hint: Think about how to design a function that doesn't need a fixed number of parameters.}
...
\texttt{utils.py} & Input helpers (validation), formatting helpers. These should be \textbf{pure utility functions} that work independently of any specific ledger. \\
...
% 8. Quality checklist
% =========================
\newpage\section{Quality Checklist (What we look for)}
