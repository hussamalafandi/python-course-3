...
\begin{enumerate}[label=\textbf{3.\arabic*}]
  \item \textbf{(1 pt)} The following code should print ``Hello, World!'' but contains an error:
        \begin{lstlisting}
message = "Hello, World!
print(message)
    \end{lstlisting}
  ...
  \item \textbf{(2 pts)} The following function should return the sum of all numbers in a list:
        \begin{lstlisting}
...
students = {
    ...
    \end{lstlisting}
        ...
        \begin{enumerate}[label=\alph*)]
          \item Add a new student ``Eve'' with grade 88:
                \vspace{1.5cm}

          \item Print all students who scored above 80:
                \vspace{2.5cm}
        \end{enumerate}

  \item \textbf{(2 pts)} What is the difference between a \textbf{list} and a \textbf{tuple}? Give one practical use case for each.
        \vspace{3.5cm}

  \item \textbf{(2 pts)} What will be the contents of \texttt{my\_set} after executing the following code?
        \begin{lstlisting}
...
\begin{enumerate}[label=\textbf{7.\arabic*}]
  \item \textbf{(3 pts)} Complete the following function that takes a list of numbers and returns a new list containing only the even numbers:
        \begin{lstlisting}
...
print(f"result_b: {result_b}")
    \end{lstlisting}
        ...
        \newpage
  \item \textbf{(3 pts)} Write a function called \texttt{count\_vowels} that takes a string and returns the number of vowels (a, e, i, o, u) in it. The function should work for both uppercase and lowercase letters.
...
print(f"Sports: {room2.messages}")
  ...
  \item \textbf{(4 pts)} Complete the \texttt{Playlist} class with proper encapsulation. The songs list should be private (use \texttt{\_songs}) and only accessible through a property that returns a copy (to prevent external modification).
        \begin{lstlisting}
class Playlist:
...
print(isinstance(notif, Notification))
    \end{lstlisting}
        ...
        \begin{enumerate}[label=\alph*)]
          ...
          \item Trace the \texttt{super().send()} calls: which methods are called and in what order when \texttt{notif.send()} executes?
                \vspace{2cm}

          \item Why does \texttt{UrgentEmail} not need its own \texttt{\_\_init\_\_} method?
                \vspace{2cm}
        \end{enumerate}
\end{enumerate}
