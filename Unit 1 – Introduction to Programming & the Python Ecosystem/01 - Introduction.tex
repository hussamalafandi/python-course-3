...
\tikzstyle{process}   = [rectangle, draw, minimum width=3.1cm, minimum height=0.85cm, align=center]
...
\begin{frame}{Flowchart-Beispiel: Zahl klassifizieren}
  ...
  \begin{FitTikZ}[node distance=0.95cm]
    ...
    \node (pos) [process, right=2.2cm of d1] {Ausgabe: ``positiv''};
    \node (d2) [decision, below=of d1, yshift=-0.2cm] {$n = 0$?};
    \node (zero) [process, right=2.2cm of d2] {Ausgabe: ``null''};
    ...
    \draw [arrow] (pos.east) -- ++ (0.8,0) |- (stop.east);

    \draw [arrow] (d1) -- node[left]{Nein} (d2);

    \draw [arrow] (d2) -- node[above]{Ja} (zero);
    \draw [arrow] (zero.east) -- ++ (0.6,0) |- (stop.east);
...
\begin{frame}{Flowchart-Übung: Login mit Wiederholung}
  \centering
  \begin{FitTikZ}[node distance=0.9cm]
    \node (start) [startstop] {Start};
    ...
    \draw [arrow] (ok.east) -- ++ (0.8,0) |- (stop.east);

    \draw [arrow] (check) -- node[left]{Nein} (inc);
    \draw [arrow] (inc) -- (limit);

    \draw [arrow] (limit) -- node[above]{Nein} (fail);
    \draw [arrow] (fail.east) -- ++ (0.6,0) |- (stop.east);

    \draw [arrow] (limit.west) -- ++ (-1.3,0) |- node[pos=0.25,left]{Ja} (input.west);
  \end{FitTikZ}
\end{frame}
